...
void lookupUserProfile(const String& uid, String& name, String& role, String& roll) {
  ...
  if (uid == "pending_sync" || uid == "sql_config" || uid == "sql_transferred" || uid == "weather_city") {
    name = "System";
    role = "SYSTEM";
    roll = "-";
    return;
  ...
  if (LittleFS.exists(path)) {
    ...
    if (f) {
      ...
      // Resolve redirect LINK| pointer if present
      if (c.startsWith("LINK|")) {
        String targetUid = c.substring(5);
        targetUid.trim();
        lookupUserProfile(targetUid, name, role, roll);
        return;
      }
