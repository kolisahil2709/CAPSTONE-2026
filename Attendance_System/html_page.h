...
.login-header-logo {
  ...
  background: linear-gradient(135deg, var(--primary), #63dfc2);
  border-radius: 20px;
  display: flex; align-items: center; justify-content: center;
  margin: 0 auto 20px;
  font-size: 32px;
...
.login-card h2 { 
  color:#fff; margin-bottom:4px; font-size: 26px; font-weight: 800; letter-spacing: -0.5px; 
}
.login-card p { 
  color:var(--text-muted); font-size:14px; margin-bottom:30px; font-weight: 500;
}
.input-group {
  position: relative;
  text-align: left;
...
<div id="login-page">
  ...
  <div class="login-card">
    ...
    <div class="input-group">
      ...
      <div class="input-field-wrapper">
        <span class="input-icon">🔒</span>
        <input type="password" id="password" value="admin123" onkeydown="if(event.key==='Enter')doLoginEnhanced()">
        <span class="password-toggle" id="pass-toggle" onclick="togglePassVisibility()">👁️</span>
      </div>
    </div>
...
<div id="main-app" style="display:none;">
  ...
  <nav>
    <div class="tab active" id="t-dir" onclick="show('dir')">👥 Employees</div>
  ...
  <div class="container">
    ...
    <!-- EMPLOYEES PAGE -->
    <div id="p-dir" class="page active">
      <div class="stats-row">
        <div class="stat-card">
        ...
        <div class="stat-card green">
          <h4>Currently In</h4>
          <div class="val" id="stat-present">—</div>
          <div class="sub">Active on premises</div>
        </div>
        ...
        <div class="card" style="flex:1; min-width:min(100%, 600px);">
          ...
          <div class="toolbar">
            ...
            <button class="btn btn-outline" onclick="syncProfilesToPC()">🔄 Sync Profiles to PC</button>
            <button class="btn btn-primary" onclick="openAddTestModal()">➕ Add Test Employee</button>
          </div>
          <div style="overflow-x:auto;">
            <table>
              <thead><tr><th>Status</th><th>ID</th><th>Name</th><th>Role</th><th>Department</th><th>Type</th><th>Action</th></tr></thead>
              <tbody id="dir-body"><tr><td colspan="7" style="text-align:center;padding:30px;color:var(--text-muted);">Loading...</td></tr></tbody>
            </table>
          </div>
        </div>
    </div>
      ...
      <div class="card">
        ...
        <div class="toolbar">
          ...
          <div style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap;">
            ...
            <button class="btn btn-outline" style="color:var(--success);border-color:var(--success);" onclick="syncLogsToPC()">🔄 Sync Logs to PC</button>
    ...
    <div id="p-reg" class="page">
      ...
      <div class="command-center">
        ...
        <!-- LEFT: SENSOR INTELLIGENCE (Hidden for RFID-only simple mode) -->
        <div class="cc-side" style="display: none !important;">
          <div class="cc-card">
          ...
          <div class="cc-card" style="flex:1; display:flex; flex-direction:column; align-items:center; justify-content:flex-start; text-align:center; position:relative; padding-top:40px;">
            ...
            <div style="max-width:400px; width:100%; margin:0 auto;">
              ...
              <div id="cc-setup-area">
                <div style="margin-bottom:20px; text-align:left; display: none !important;">
                  ...
                  <select id="enroll-method" onchange="toggleEnrollMethod()" style="width:100%; font-size:16px; padding:10px; border-radius:8px; background:var(--bg); border:1px solid var(--border); color:var(--text); font-weight:600; outline:none; height:46px;">
                    <option value="RFID" selected>RFID Card Only (No Fingerprint)</option>
                    <option value="FINGER">Fingerprint + RFID Card (2-Factor / Biometric)</option>
                  </select>
                </div>
        ...
        <div class="cc-side">
          ...
          <div class="cc-card" id="cc-details-card" style="flex:1; opacity:0.3; pointer-events:none; transition:0.5s;">
            ...
            <div class="enroll-input-group" style="display: none;">
    ...
    <div id="p-set" class="page">
      <div class="settings-grid">
        ...
        <div class="card">
          ...
          <p class="hint">Configure your backend server URL for database logging.</p>
          
          <label>Webhook API URL</label>
          <input type="text" id="set-webhook" placeholder="e.g. http://192.168.1.5:3000/webhook/scan">
          
          <div class="card-footer">
            <button class="btn btn-primary" style="width:100%;" onclick="saveWebhookSettings()">💾 Save Webhook URL</button>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div style="display:flex; flex-direction:column; gap:12px; margin-top:10px;">
            ...
            <div style="border-top:1px solid var(--border); margin-top:8px; padding-top:12px; display:flex; flex-direction:column; gap:8px;">
              ...
              <button class="btn btn-outline btn-sm" style="color:var(--danger); border-color:var(--danger); justify-content:center; display:flex; align-items:center; gap:6px; font-size:12px; display: none !important;" onclick="wipeBiometricsOnly()">👆 Wipe Biometric Templates</button>
...
<div id="edit-modal" class="modal">
  <div class="modal-content">
    ...
    <div id="edit-upgrade-container" style="display:none !important; margin-top:15px; border-top:1px solid var(--border); padding-top:15px;">
...
<!-- ADD TEST EMPLOYEE MODAL -->
<div id="add-test-modal" class="modal">
  <div class="modal-content">
    <h3 style="margin-top:0;">Add Test Employee</h3>
    <label>Employee ID (Number)</label>
    <input type="number" id="add-roll" placeholder="e.g. 101">
    <label>Unique UID (Text/Number)</label>
    <input type="text" id="add-uid" placeholder="e.g. 9999">
    <label>Full Name</label>
    ...
    <select id="add-type" onchange="toggleAddRfidField()">
      <option value="RFID">RFID Card Only</option>
      <option value="FINGER">Fingerprint Only</option>
      <option value="BOTH">Fingerprint + RFID Card</option>
    </select>
    <div id="add-rfid-container" style="margin-top:10px;">
      <label>RFID Card UID (Optional)</label>
      <input type="text" id="add-rfid" placeholder="e.g. AABBCCDD">
    </div>
    <div class="row" style="margin-top:20px;">
      <button class="btn btn-primary" style="flex:1;" onclick="submitAddTestEmployee()">Save Employee</button>
      <button class="btn btn-outline" onclick="closeAddTestModal()">Cancel</button>
    </div>
  </div>
</div>
...
var storage = {
  getItem: function(key) {
    try {
      return (typeof window !== 'undefined' && window.localStorage) ? window.localStorage.getItem(key) : (this[key] || null);
    } catch(e) {
...
function enterApp(data) {
  ...
  if (data) {
    ...
    safeFetch('/init-data')
      .then(function(r){ return r.json(); })
      .then(processData)
      .catch(function(){ setupWebSocket(); show('dir'); });
  }
}
...
function renderDirectoryTable() {
  ...
  if(users.length === 0) tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:30px;color:var(--text-muted);">No employees registered yet.</td></tr>';
  else filterDir();
}
...
function openAddTestModal() {
  ...
  document.getElementById('add-type').value = 'RFID';
  document.getElementById('add-rfid').value = '';
  document.getElementById('add-rfid-container').style.display = 'block';
  var am = document.getElementById('add-test-modal');
...
function toggleAddRfidField() {
  var type = document.getElementById('add-type').value;
  var rfidContainer = document.getElementById('add-rfid-container');
  if (type === 'FINGER') {
...
function submitAddTestEmployee() {
  ...
  var type = document.getElementById('add-type').value;
  var rfid = document.getElementById('add-rfid').value.trim();

  if (!roll || !uid || !name) {
    showToast("Please fill in ID, UID, and Name!", "error");
    return;
  }

  var actualType = (type === 'RFID') ? 'RFID' : 'FINGER';
  
  var url = '/save?uid=' + encodeURIComponent(uid) +
  ...
  safeFetch(url).then(function(r) {
    if (r.ok) {
      showToast("Test employee added successfully!", "success");
      closeAddTestModal();
      loadDirectory();
    } else {
      showToast("Failed to add test employee", "error");
    }
  }).catch(function(err) {
    showToast("Network error", "error");
  });
}
function syncProfilesToPC() {
  showToast("Syncing all employee profiles to PC...", "info");
  safeFetch('/sync-all-users').then(function(r) {
    if (r.ok) {
      showToast("Profiles synced successfully to PC database!", "success");
    } else {
...
function syncLogsToPC() {
  ...
  var webhookVal = document.getElementById('set-webhook').value;
  if (!webhookVal || webhookVal.length < 10) {
    return showToast("Please configure Webhook URL first", "error");
  }
  var baseEnd = webhookVal.indexOf('/', 7);
  var baseUrl = baseEnd !== -1 ? webhookVal.substring(0, baseEnd) : webhookVal;
  var syncUrl = baseUrl + '/api/sync-logs';
  
  showToast("Fetching logs to sync...", "info");
  ...
  function fetchNextDate(index) {
    if (index >= datesToFetch.length) {
      if (allLogs.length === 0) {
        showToast("No logs found to sync in this range", "info");
        return;
      }
      
      showToast("Sending " + allLogs.length + " logs to database...", "info");
      ...
      .then(function(data) {
        showToast("Successfully synced " + (data.inserted || 0) + " new logs (" + (data.skipped || 0) + " skipped duplicates)!", "success");
      })
      .catch(function(err) {
        console.error(err);
        showToast("Sync failed! Check backend server status/CORS.", "error");
      });
      return;
    }
      ...
      .then(function(text) {
        if(text && text.trim()) {
          ...
          for (var i = 0; i < lines.length; i++) {
            ...
            var timeStr = cols[0].trim();
            var nameStr = cols[2].trim();
            var roleStr = isOldFormat ? "-" : cols[3].trim();
            var dirStr = cols[4].trim();
            var statusDisp = isOldFormat ? cols[4].trim() : cols[5].trim();
            
            var timestamp = formattedDate + ' ' + timeStr;
...
window.onload = function() {
  try {
    ...
    safeFetch('/init-data')
      .then(function(r) { return r.json(); })
      .then(function(data) {
        var isAP = data.isAPMode === true || (data.ip && (data.ip.indexOf("AP:") === 0 || data.ip.indexOf("192.168.4.1") !== -1));
        if (isAP || storage.getItem("loggedIn") === "true") {
          enterApp(data);
        } else {
          var lp = document.getElementById('login-page');
          var ma = document.getElementById('main-app');
...
function saveWebhookSettings() {
  ...
  if (webhook.length === 0) {
    showToast("Webhook URL cannot be empty!", "error");
    return;
  }
  showToast("Saving Webhook URL...", "info");
  safeFetch('/set-branding?webhook=' + encodeURIComponent(webhook))
    .then(function(r) {
      if (r.ok) {
