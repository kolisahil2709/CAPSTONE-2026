...
void sendSyncRequest(String path, String json) {
  if (webhookUrl.length() < 10)
    return;
  if (WiFi.status() != WL_CONNECTED)
    return;

  // Derive base URL from webhookUrl (e.g. "http://192.168.1.100:3000")
  int firstSlash = webhookUrl.indexOf('/', 7); // Skip "http://"
  if (firstSlash == -1) return;
  String baseUrl = webhookUrl.substring(0, firstSlash);
  String syncUrl = baseUrl + path;

  SyncPayload *payload = new SyncPayload{syncUrl, json};

  xTaskCreate(
      [](void *p) {
        SyncPayload *pPayload = (SyncPayload *)p;
        HTTPClient http;
        http.begin(pPayload->url);
        http.addHeader("Content-Type", "application/json");
        int code = http.POST(pPayload->data);
        http.end();
        delete pPayload;
        vTaskDelete(NULL);
      },
      "sync_task", 4096, payload, 1, NULL);
}

bool isAuth(AsyncWebServerRequest *request) {
  if (isAPMode) return true; // Bypass in local AP Mode
...
void setupDashboard() {
  ...
  server.on("/api/login", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    if (username == String(loginUser) && password == String(loginPass)) {
      ...
      response->addHeader("Set-Cookie", "session=admin_active; Path=/; Max-Age=86400; HttpOnly");
  ...
  server.on("/save", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    // Sync employee registration to PC SQLite database
    String syncUid = cleanUid + (type == "FINGER" ? "_f" : "");
    String syncJson = "{\"uid\":\"" + syncUid + 
                      "\",\"name\":\"" + request->getParam("name")->value() + 
                      "\",\"role\":\"" + request->getParam("role")->value() + 
                      "\",\"roll\":\"" + request->getParam("roll")->value() + 
                      "\",\"type\":\"" + type + 
                      "\",\"rfid\":\"" + rfidVal + 
                      "\",\"dept\":\"" + dept + "\"}";
    sendSyncRequest("/api/sync-employee", syncJson);
  ...
  server.on("/delete-user", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (removed) {
      updateFsSizes();
      // Sync delete request to PC SQLite database
      String syncJson = "{\"uid\":\"" + uid + "\"}";
      sendSyncRequest("/api/delete-employee", syncJson);
      request->send(200, "text/plain", "OK");
    } else
  ...
  server.on("/edit-user", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (LittleFS.exists(path)) {
      ...
      if (fw) {
        ...
        // Sync edited profile to PC SQLite database
        String syncJson = "{\"uid\":\"" + uid + 
                          "\",\"name\":\"" + name + 
                          "\",\"role\":\"" + role + 
                          "\",\"roll\":\"" + roll + 
                          "\",\"type\":\"" + (isFingerprint ? "FINGER" : "RFID") + 
                          "\",\"rfid\":\"" + newRfid + 
                          "\",\"dept\":\"" + dept + "\"}";
        sendSyncRequest("/api/sync-employee", syncJson);
  ...
  // Bulk sync route to retrieve all profiles from ESP32 to SQL Database
  server.on("/sync-all-users", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!isAuth(request)) return request->send(403, "text/plain", "Unauthorized");
    ...
    while (file) {
      ...
      if (fn.endsWith(".txt") && fn != "wifi.txt" && fn != "wifi.conf" && fn != "config.sys") {
        ...
        if (pCount >= 3) {
          ...
          String role = (pCount >= 2) ? c.substring(pipes[0] + 1, pipes[1]) : "-";
          String roll = (pCount >= 3) ? c.substring(pipes[1] + 1, pipes[2]) : uid_clean;
          String type = fn.endsWith("_f.txt") ? "FINGER" : "RFID";
          String linkedRfid = "";
          String dept = "-";
          ...
          if (type == "FINGER") {
            ...
            if (pCount >= 7) {
              ...
              if (c.substring(pipes[2]+1, pipes[3]) == "0") {
                dept = c.substring(pipes[5] + 1);
              }
            }
          }
    ...
    sendSyncRequest("/api/sync-all-employees", json);
    request->send(200, "text/plain", "Sync initiated");
  });

  server.on("/set-branding", HTTP_GET, [](AsyncWebServerRequest *request) {
    Serial.println("📥 Received /set-branding request!");
    if (request->hasParam("webhook")) {
      webhookUrl = request->getParam("webhook")->value();
      Serial.printf("🔗 Parsed Webhook Parameter: %s\n", webhookUrl.c_str());
    }
    ...
    if (f) {
      ...
      onBrandingUpdated();
      request->send(200, "text/plain", "OK");
      Serial.println("💾 config.sys updated successfully!");
    } else {
      request->send(500, "text/plain", "Write error");
      Serial.println("❌ Failed to write config.sys!");
    }
  });
